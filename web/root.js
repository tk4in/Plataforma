...
router.get('/', (req, res, next) => {
  ...
  res.write(
    ...
    "if ('caches' in window) {caches.open('" + process.env.AppID + "-models').then((cache) => {	const url = 'https://" + process.env.CDNAddr + "/3d/cars/car-6RCup.glb';	cache.match(url).then((cachedResponse) => { if (!cachedResponse || !cachedResponse.ok) { const urls = [url, 'https://" + process.env.CDNAddr + "/3d/air/air-zVuA.glb', 'https://" + process.env.CDNAddr + "/3d/air/air-oBEWt.glb'];	cache.addAll(urls).then(() => console.log('Data added to cache.')).catch((error) => console.error('Error adding data to cache:', error));}}); }); }");
